...
let passwordDigest = require('../utils').passwordDigest;
let validPasswordTypes = ['PasswordDigest', 'PasswordText'];
...
class WSSecurity {
  ...
  toXML() {
    ...
    let now = new Date();
    let created = getDate(now);
    let timeStampXml = '';
    if (this._hasTimeStamp) {
      let expires = getDate(new Date(now.getTime() + (1000 * 600)));
    ...
    if (this._passwordType === 'PasswordText') {
      ...
      password = `<wsse:Password Type="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-username-token-profile-1.0#PasswordDigest">
