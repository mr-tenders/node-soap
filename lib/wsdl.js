...
let NamespaceContext = require('./nscontext');
let fs = require('fs');
let url = require('url');
let path = require('path');
let assert = require('assert').ok;
let stripBom = require('strip-bom');
let debug = require('debug')('node-soap');
...
SimpleTypeElement.prototype.description = function() {
  let children = this.children;
  for (let i = 0, child; child = children[i]; i++) {
...
let WSDL = function(definition, uri, options) {
  let self = this,
      fromFunc;
...
WSDL.prototype.xmlToObject = function(xml, callback) {
  ...
  function finish() {
    ...
    if (root.Envelope) {
      ...
      if (body && body.Fault) {
        let code = selectn('faultcode.$value', body.Fault) || selectn('faultcode', body.Fault);
        let string = selectn('faultstring.$value', body.Fault) || selectn('faultstring', body.Fault);
        let detail = selectn('detail.$value', body.Fault) || selectn('detail.message', body.Fault);
