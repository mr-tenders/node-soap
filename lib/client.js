...
let Client = function(wsdl, endpoint, options) {
...
Client.prototype._invoke = function(method, args, location, callback, options, extraHeaders) {
  ...
  //Add extra headers
  for (let header in this.httpHeaders ) { headers[header] = this.httpHeaders[header];  }
  for (let attr in extraHeaders) { headers[attr] = extraHeaders[attr]; }
  ...
  let eid = options.exchangeId || uuid.v4();

  self.emit('message', message, eid);
  self.emit('request', xml, eid);

  let tryJSONparse = function(body) {
    try {
  ...
  function parseSync(body, response) {
    ...
    try {
      ...
      if(!output || !output.$lookupTypes) {
        ...
        let json = _.isObject(body) ? body : tryJSONparse(body);
        if (json) {
  ...
  function finish(obj, body, response) {
    ...
    if( typeof obj.Body !== 'object' ) {
      let error = new Error('Cannot parse response');
      error.response = response;
      error.body = body;
