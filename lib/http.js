...
HttpClient.prototype.buildRequest = function(rurl, data, exheaders, exoptions) {
  let curl = url.parse(rurl);
  let secure = curl.protocol === 'https:';
  let host = curl.hostname;
  let port = parseInt(curl.port, 10);
  let path = [curl.pathname || '/', curl.search || '', curl.hash || ''].join('');
